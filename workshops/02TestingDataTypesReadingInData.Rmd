...
![R](../pics/R.png) Make an RStudio project for this workshop by clicking on the drop-down menu on top right where it says [Project: (None)](../pics/new-project.png) and choosing New Project and then New Directory. Navigate to the "data-analysis-in-r" folder. Name the RStudio Project 'workshop2.
...
A column is referred to using either:
...
2. or the dollar notation `pigeon$distance`. 
...
Individual values can be extracted by indexing - giving the row number and column number of the element you want: `pigeon[row, col]`.

![R](../pics/R.png) Extract the 14th value in the column? 
```{r echo=FALSE}
#---CODING ANSWER---
pigeon$distance[14]
#or
...
It is good practice to start axes at zero and we can achieve that using `scale_x_continuous()`.

![R](../pics/R.png) Start *x*-axis at zero and finish it at 15. I'll also alter the number of bins to account for the bigger range.
```{r}
ggplot(data = pigeon, aes(x = distance)) +
...
![R](../pics/R.png) Find out which functions will calculate the mean, standard deviation and the number of cases, *i.e.*, the length of the vector. Google or make intelligent guesses and verify with the manual.
...
![R](../pics/R.png) To read the data in to R you need to use the relative path to the file in the `read-table2()` command:
...
Now we will create a word document with some text and figures. I find the best way to include figures in a word document is to use a table with the borders turned off. This can make it easier to control the layout. This is what you are aiming for: [pdf of word doc](pigeon.pdf)
...
1. Myoglobin concentration in seals
