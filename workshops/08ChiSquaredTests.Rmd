...
The data we were given was already tabulated however, data often comes to us with an observation in each row. In this case that would mean a column of 250 values which indicated which day and induction was performed. Data formatted this way is in [inductions.txt](../data/inductions.txt).
...
![W](../pics/W.png) Save a copy of [inductions.txt](../data/inductions.txt) to the `data-raw` folder

![R](../pics/R.png) Read the data in to R. Remember, you need to use the relative path to the file in the `read.table()` command and decide whether you need header to be TRUE or FALSE:
...
Remember - the ggplot geom_bar() will count the number of each day for us which is great! `r emo::ji("party")`

What is not so great is the alphbetical ordering `r emo::ji("annoyed")`

```{r}
 levels(inductions$day)
```

![R](../pics/R.png) Reordering factor levels is something we quite often want to do.
...
![R](../pics/R.png) Redraw the figure.
...
![W](../pics/W.png) Save a copy of [blood_ulcers.txt](../data/blood_ulcers.txt) to the 'data' folder
...
![R](../pics/R.png) look at the structure of the dataframe using `str()`
...
![R](../pics/R.png) Examine the 'structure' of the `obs` object using `str()`
...
![R](../pics/R.png) Carry out the appropriate chi-squared test on these data either with or withour using inbuilt test, whatever you prefer.
